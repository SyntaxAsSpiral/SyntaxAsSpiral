...
### Core Components
- **`src/pulse_rotator.py`**: Main orchestrator. Generates pulse content, builds data dict, renders HTML via template, archives logs, commits and pushes changes.
...
3. Modify `src/pulse_rotator.py` for HTML generation logic
...
3. Generate field in `pulse_rotator.py` and add to `pulse_data` dict
...
- Test with git push: `python src/pulse_rotator.py`
...
- **Content**: Generated by `pulse_generator.py` + `pulse_rotator.py` → `pulse.json`
